...
public class StatusesAPI extends AbsOpenAPI {
    ...
    public void readOne(long id, RequestListener listener) {
        WeiboParameters params = buildReadOneParams(id);
        requestAsync(SERVER_URL_PRIX + "/show.json", params, HTTPMETHOD_GET, listener);
    }

    // 组装微博请求参数
    private WeiboParameters buildReadOneParams(long id) {
        WeiboParameters params = new WeiboParameters(mAppKey);
        params.put("id", id);
        return params;
    }
